(function (window, $) {
    var server = 'http://query.china.com.cn/news/queryFn',
        ...
        render_data = function (data) {
            ...
            $('.searchKey').html('<div>在<b>关键词</b>中搜索<b class="search-word"></b></div>');
    ...
    //时间插件事件
    $('.adv-fromdate').on('change', function () {
        var f_year = this.value.substr(12),
            f_month = this.value.substr(6, 2),
            f_day = this.value.substr(9, 2),
            f_time = this.value.substr(0, 5),
            fromD = f_year + '-' + f_month + '-' + f_day + ' ' + f_time;
    ...
    $('.adv-todate').on('change', function () {
        var t_year = this.value.substring(12),
            t_month = this.value.substr(6, 2),
            t_day = this.value.substr(9, 2),
            t_time = this.value.substr(0, 5),
            toD = t_year + '-' + t_month + '-' + t_day + ' ' + t_time;
        param['toDate'] = toD;
        if (toD == '-- ') {
            delete(param['toDate']);
        }
...
(function () {
    // core function
    var CIIC_SEARCH = {}; //window.CIIC_SEARCH || {};

    CIIC_SEARCH.execute = function () {};
    CIIC_SEARCH.render_data = function () {};

    window.CIIC_SEARCH = CIIC_SEARCH;
}());

(function () {
    // Simple search
    var CIIC_SEARCH = window.CIIC_SEARCH || false;
    if (!CIIC_SEARCH) {
        throw new Error('Fail to load core function library!');
    }
